...
export default async function ProjectPage({ params }: Props) {
  const slug = params.slug; // ✅ Sem await aqui
  ...
  return (
    <main style={{ padding: "2rem", maxWidth: "1000px", margin: "0 auto" }}>
      <div style={{ marginBottom: "1rem" }}>
        <Link href="/">
          <button style={{ padding: "0.5rem 1rem", backgroundColor: "#0d6efd", color: "#fff", border: "none", borderRadius: "5px", cursor: "pointer" }}>
            ← Back to Main Page
          </button>
        </Link>
      </div>
      ...
      <table style={{ width: "100%", marginTop: "2rem", borderCollapse: "collapse" }}>
        <thead>
          <tr style={{ background: "#f0f0f0" }}>
            <th style={{ border: "1px solid #ccc", padding: "0.5rem" }}>Law Title</th>
        ...
        <tbody>
          {projects.length > 0 ? (
            projects.map((project: any, idx: number) => (
              <tr key={idx}>
                <td style={{ border: "1px solid #ccc", padding: "0.5rem" }}>{project.title}</td>
                <td style={{ border: "1px solid #ccc", padding: "0.5rem" }}>{project.status}</td>
